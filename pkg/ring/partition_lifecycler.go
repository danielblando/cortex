...
// PartitionLifecycler manages the lifecycle of an ingester in the partition ring.
type PartitionLifecycler struct {
	*services.BasicService
	...
	// We need to remember the ingester state and registered timestamp just in case the KV store
...
func NewPartitionLifecycler(
	...
	// Determine partition ID - use configured value or default to p-0
	partitionID, err := AutoSelectPartition(cfg.ID)
	if err != nil || partitionID == "" {
		level.Info(logger).Log("msg", "failed to find partition")
		partitionID = "p-0"
	}

	l := &PartitionLifecycler{
		cfg:                  cfg,
		flushTransferer:      flushTransferer,
		KVStore:              store,
...
func (i *PartitionLifecycler) loop(ctx context.Context) error {
	joined := false
	...
	for {
		...
		case <-observeChan:
			...
			observeChan = nil
			if s := i.GetInstanceState(); s != JOINING {
...
func (i *PartitionLifecycler) initRing(ctx context.Context) (bool, error) {
	var (
	...
	err = i.KVStore.CAS(ctx, i.RingKey, func(in interface{}) (out interface{}, retry bool, err error) {
		...
		partitionDesc, ok := ringDesc.Partitions[i.partitionID]
		if !ok {
			// The partition doesn't exist in the ring, so it's safe to set the registered timestamp
			// as of now.
			...
			if len(tokensFromFile) > 0 {
				...
				if len(tokensFromFile) >= i.cfg.NumTokens && i.autoJoinOnStartup {
					i.setInstaceState(i.getInstancePreviousState())
					state := i.GetInstanceState()
					partition := ringDesc.AddPartition(i.partitionID, i.GetPartitionState(), tokensFromFile, registeredAt)
					partition.AddIngester(i.ID, i.Addr, i.Zone, i.GetInstanceState())
			...
			level.Info(i.logger).Log("msg", "partition not found in ring, adding with no tokens", "ring", i.RingName)
			partition := ringDesc.AddPartition(i.partitionID, i.GetPartitionState(), []uint32{}, registeredAt)
			partition.AddIngester(i.ID, i.Addr, i.Zone, i.GetInstanceState())
			return ringDesc, true, nil
		}
		...
		i.setRegisteredAt(partitionDesc.GetRegisteredAt())
		i.setPartitionState(partitionDesc.State)

		instanceDesc, ok := partitionDesc.Instances[i.ID]
		if !ok {
			//The instance does not exist in the partition
			instanceDesc = partitionDesc.AddIngester(i.ID, i.Addr, i.Zone, i.GetInstanceState())
			return ringDesc, true, nil
		}
		...
		i.setInstaceState(instanceDesc.State)
		tokens, _ := ringDesc.TokensFor(i.partitionID)
		i.setTokens(tokens)

		level.Info(i.logger).Log("msg", "existing entry found in ring", "partition state", i.GetPartitionState(), "tokens", len(tokens), "ring", i.RingName)
		...
		if i.cfg.HeartbeatPeriod == 0 && (!partitionDesc.Equal(ringDesc.Partitions[i.partitionID]) || !instanceDesc.Equal(partitionDesc.Instances[i.ID])) {
			// Update timestamp to give gossiping client a chance register ring change.
			instanceDesc.Timestamp = time.Now().Unix()
			ringDesc.Partitions[i.partitionID].Instances[i.ID] = instanceDesc
			return ringDesc, true, nil
		}
...
func (i *PartitionLifecycler) heartbeat(ctx context.Context) {
...
func (i *PartitionLifecycler) stopping(runningError error) error {
	if runningError != nil {
	...
	if i.cfg.TokensFilePath != "" {
		currentState := i.GetInstanceState()
		i.setInstancePreviousState(currentState)
	}
	...
	if i.GetInstanceState() != READONLY {
...
func (i *PartitionLifecycler) processShutdown(ctx context.Context) {
...
func (i *PartitionLifecycler) unregister(ctx context.Context) error {
	...
	return i.KVStore.CAS(ctx, i.RingKey, func(in interface{}) (out interface{}, retry bool, err error) {
		...
		ringDesc := in.(*PartitionRingDesc)
		ringDesc.RemoveIngester(i.partitionID, i.ID)
		return ringDesc, true, nil
	})
}

// autoJoin selects random tokens & moves state to targetState
func (i *PartitionLifecycler) autoJoin(ctx context.Context, targetState InstanceState, alreadyInRing bool) error {
	...
	err := i.KVStore.CAS(ctx, i.RingKey, func(in interface{}) (out interface{}, retry bool, err error) {
		...
		if needTokens == 0 && myTokens.Equals(i.getTokens()) {
			...
			state := i.GetInstanceState()
			partitionDesc := ringDesc.AddPartition(i.partitionID, i.GetPartitionState(), i.getTokens(), i.getRegisteredAt())
			partitionDesc.AddIngester(i.ID, i.Addr, i.Zone, state)
			level.Info(i.logger).Log("msg", "auto joined with existing tokens", "ring", i.RingName, "state", state)
			return ringDesc, true, nil
		}

		newTokens := generateTokens(ringDesc, i.ID, i.Zone, needTokens, false)
...
// consul restarts.
func (i *PartitionLifecycler) updateConsul(ctx context.Context) error {
	var ringDesc *PartitionRingDesc
	...
	err := i.KVStore.CAS(ctx, i.RingKey, func(in interface{}) (out interface{}, retry bool, err error) {
		...
		if !ok {
			...
			partitionDesc = ringDesc.AddPartition(i.partitionID, P_NON_READY, i.getTokens(), i.getRegisteredAt())
		} else {
			instanceDesc, ok := partitionDesc.Instances[i.ID]
			if !ok {
				// consul must have restarted
				level.Info(i.logger).Log("msg", "found empty ring, inserting tokens", "ring", i.RingName)
				partitionDesc.AddIngester(i.ID, i.Addr, i.Zone, i.GetInstanceState())
			} else {
				instanceDesc.Timestamp = time.Now().Unix()
				partitionDesc.Timestamp = time.Now().Unix()
				instanceDesc.State = i.GetInstanceState()
				instanceDesc.Addr = i.Addr
				instanceDesc.Zone = i.Zone

				i.updatePartitionState(partitionDesc)
				partitionDesc.State = i.GetPartitionState()

				partitionDesc.Instances[i.ID] = instanceDesc
				ringDesc.Partitions[i.partitionID] = partitionDesc
			}
		}
...
func (i *PartitionLifecycler) updatePartitionState(partition *PartitionDesc) {
	state := P_ACTIVE
	if len(partition.Instances) != i.cfg.RingConfig.ReplicationFactor {
		state = P_NON_READY
	} else {
	...
	i.setPartitionState(state)
}

func (i *PartitionLifecycler) getTokens() Tokens {
	i.stateMtx.RLock()
	defer i.stateMtx.RUnlock()
	return i.tokenFile.Tokens
}

func (i *PartitionLifecycler) setTokens(tokens Tokens) {
...
func (i *PartitionLifecycler) getRegisteredAt() time.Time {
	i.stateMtx.RLock()
	defer i.stateMtx.RUnlock()
	return i.registeredAt
}

func (i *PartitionLifecycler) setRegisteredAt(registeredAt time.Time) {
	i.stateMtx.Lock()
	defer i.stateMtx.Unlock()
...
func (i *PartitionLifecycler) GetPartitionState() PartitionState {
	i.stateMtx.RLock()
	defer i.stateMtx.RUnlock()
	return i.partitionState
}

// GetState returns the state of this ingester.
func (i *PartitionLifecycler) setPartitionState(state PartitionState) {
	i.stateMtx.RLock()
	defer i.stateMtx.RUnlock()
	i.partitionState = state
}

// GetState returns the state of this ingester.
func (i *PartitionLifecycler) GetInstanceState() InstanceState {
	i.stateMtx.RLock()
	defer i.stateMtx.RUnlock()
	return i.state
}
...
// GetState returns the state of this ingester.
func (i *PartitionLifecycler) setInstaceState(state InstanceState) {
	i.stateMtx.Lock()
	defer i.stateMtx.Unlock()
...
func (i *PartitionLifecycler) getInstancePreviousState() InstanceState {
	i.stateMtx.RLock()
	defer i.stateMtx.RUnlock()
	return i.tokenFile.PreviousState
}

func (i *PartitionLifecycler) setInstancePreviousState(state InstanceState) InstanceState {
	i.stateMtx.RLock()
	defer i.stateMtx.RUnlock()
	return i.tokenFile.PreviousState
}

func (i *PartitionLifecycler) loadTokenFile() (*TokenFile, error) {
...
func (i *PartitionLifecycler) updateCounters(ringDesc *PartitionRingDesc) {
	...
	if ringDesc != nil {
		...
		for _, partition := range ringDesc.Partitions {
			...
			for _, ingester := range partition.Instances {
...
func (i *PartitionLifecycler) CheckReady(ctx context.Context) error {
	i.readyLock.Lock()
...
func (i *PartitionLifecycler) checkRingHealthForReadiness(ctx context.Context) error {
	...
	ringDesc, ok := desc.(*PartitionRingDesc)
	if !ok || ringDesc == nil {
	...
	if i.cfg.ReadinessCheckRingHealth {
		...
		p, ok := ringDesc.Partitions[i.partitionID]
		if !ok {
			return fmt.Errorf("partition %s not found in the ring", i.partitionID)
		}
		instance, ok := p.Instances[i.ID]
		if !ok {
...
func (i *PartitionLifecycler) verifyTokens(ctx context.Context) bool {
	result := false
	...
	err := i.KVStore.CAS(ctx, i.RingKey, func(in interface{}) (out interface{}, retry bool, err error) {
		...
		ringTokens, _ := ringDesc.TokensFor(i.partitionID)
		...
		if !i.compareTokens(ringTokens) {
			...
			newTokens := generateTokens(ringDesc, i.ID, i.Zone, needTokens, true)

			ringTokens = append(ringTokens, newTokens...)
			sort.Sort(ringTokens)

			ringDesc.AddPartition(i.partitionID, i.GetPartitionState(), ringTokens, i.getRegisteredAt())
...
func (i *PartitionLifecycler) compareTokens(fromRing Tokens) bool {
	sort.Sort(fromRing)
...
func generateTokens(ring *PartitionRingDesc, _, _ string, numTokens int, _ bool) []uint32 {
	if numTokens <= 0 {
		return []uint32{}
	}
...
func (i *PartitionLifecycler) HealthyInstancesCount() int {
...
// in the ring.
func (i *PartitionLifecycler) ZonesCount() int {
	i.countersLock.RLock()
	defer i.countersLock.RUnlock()
...
func (i *PartitionLifecycler) GetZone() string {
	return i.Zone
}

// ShouldUnregisterOnShutdown returns if unregistering should be skipped on shutdown.
func (i *PartitionLifecycler) ShouldUnregisterOnShutdown() bool {
	return i.unregisterOnShutdown.Load()
}

// FlushOnShutdown returns if flushing is enabled if transfer fails on a shutdown.
func (i *PartitionLifecycler) FlushOnShutdown() bool {
	return i.flushOnShutdown.Load()
}

// GetInstanceAddr returns the instance address.
func (i *PartitionLifecycler) GetInstanceAddr() string {
	return i.Addr
}

// GetInstanceID returns the instance ID.
func (i *PartitionLifecycler) GetInstanceID() string {
	return i.ID
}

// GetState returns the state of this ingester.
func (i *PartitionLifecycler) GetState() InstanceState {
	i.stateMtx.RLock()
	defer i.stateMtx.RUnlock()
	return i.state
}
...
func (i *PartitionLifecycler) IsRegistered() (bool, error) {
	...
	partition, exists := ringDesc.Partitions[i.partitionID]
	if !exists {
		return false, nil
	}
	_, exists = partition.Instances[i.ID]
	return exists, nil
}

// Join trigger the instance to join the ring, if autoJoinOnStartup is set to false.
func (i *PartitionLifecycler) Join() {
	select {
...
func (i *PartitionLifecycler) RenewTokens(ratio float64, ctx context.Context) {
	if ratio > 1 {
		ratio = 1
	}
	err := i.KVStore.CAS(ctx, i.RingKey, func(in interface{}) (out interface{}, retry bool, err error) {
		...
		ringDesc := in.(*PartitionRingDesc)
		_, ok := ringDesc.Partitions[i.partitionID]
...
// AutoSelectPartition determines which partition an instance should join based on its ID.
// It extracts a number from the instance ID suffix and returns the corresponding partition ID.
// For example, if the instance ID is "ing-az1-3", it will return "p-3".
// If no number can be extracted, it returns an error.
func AutoSelectPartition(instanceID string) (string, error) {
	// Try to extract a number from the instance ID suffix
	re := regexp.MustCompile(`-(\d+)$`)
	matches := re.FindStringSubmatch(instanceID)

	if len(matches) < 2 {
		return "", fmt.Errorf("could not extract partition number from instance ID: %s", instanceID)
	}

	// Parse the extracted number
	partitionNum, err := strconv.Atoi(matches[1])
	if err != nil {
		return "", fmt.Errorf("failed to parse partition number from instance ID: %s, error: %w", instanceID, err)
	}
