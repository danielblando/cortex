...
func (d *Desc) RefreshTimestamp() {
	var fresherTimestamp int64
	for _, ing := range d.Ingesters {
		if ing.Timestamp > fresherTimestamp {
			fresherTimestamp = ing.Timestamp
		}
	}
	staleDuration := time.Now().UTC().Unix() - fresherTimestamp
	for id, ing := range d.Ingesters {
		ing.Timestamp += staleDuration
		d.Ingesters[id] = ing
	}
}
